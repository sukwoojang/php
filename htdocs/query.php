...
if ($cnt != 0) {
	...
	print "<table border = 1><tr><td><b><center>학번</center></b></td><td><b><center>성명</center></b></td><td>성별</td><td>권한</td><td>등록일</td><td>수정일</td><td><b><center>삭제</center></b></td><td><b>수정</b></td></tr>";
	for ($i = 0; $i < $cnt ;$i++){#for문 용법 알아두자
		...
		elseif($row[auth] == '')
			$disp_auth = '미승인';
		print "<form name = delete method = post action = '$PHP_SELF?flag=D'><tr><td><center>$row[stud_id]</center></td><td><center>$row[stud_name]</center></td><td>$disp_gen</td><td>$disp_auth</td><td>$row[reg_date]</td><td>$row[upd_date]</td>"; #mysql_fetch_array를 사용하였기때문에 $row[]내부에 인덱스값이나 열이름을 쳐도 둘다 된다.
		print "<td><input type = submit value = '삭제'></td><td><a href='update.php?u_stud_id=$row[stud_id]' target='home'>수정</a></td></tr><input type = hidden name = 'id' value = $row[stud_id]></form>";
	}
	print "</table>";
